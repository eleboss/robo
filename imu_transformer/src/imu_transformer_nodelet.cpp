...
namespace imu_transformer
{
  ...
  void ImuTransformerNodelet::onInit(){
    ...
    private_nh_ = getPrivateNodeHandle();
      ...
      // **** get paramters
    if (!private_nh_.getParam ("in_prefix", in_prefix_))
      in_prefix_ = "px4/imu";
    if (!private_nh_.getParam ("out_prefix", out_prefix_))
      out_prefix_ = "robo/imu";
    if (!private_nh_.getParam ("out_link", out_link_))
      out_link_ = "base_link";

    nh_in_ = ros::NodeHandle(getNodeHandle(), in_prefix_);
    nh_out_ = ros::NodeHandle(getNodeHandle(), out_prefix_);

    private_nh_.param<std::string>("target_frame", target_frame_, out_link_);
...
//      mag_filter_->registerFailureCallback(boost::bind(&ImuTransformerNodelet::failureCb, this, _2));
  ...
  void ImuTransformerNodelet::imuCallback(const ImuMsg::ConstPtr &imu_in)
  {
    if(imu_pub_.getTopic().empty()){
      imu_pub_ = nh_out_.advertise<ImuMsg>("data", 1);
    }
    ...
    try
    {
      ...
      imu_out.header.frame_id = out_link_;
